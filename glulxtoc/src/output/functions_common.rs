...
impl GlulxOutput {
    ...
    pub fn output_common_instruction(&self, instruction: &Instruction, args: Vec<String>) -> String {
        ...
        match opcode {
            ...
            OP_ALOAD => format!("Mem4({} + 4 * (glsi32) {})", op_a, op_b),
            OP_ALOADS => format!("Mem2({} + 2 * (glsi32) {})", op_a, op_b),
            OP_ALOADB => format!("Mem1({} + (glsi32) {})", op_a, op_b),
            OP_ALOADBIT => self.runtime("OP_ALOADBIT", args),
            OP_ASTORE => format!("store_operand(1, {} + 4 * (glsi32) {}, {})", op_a, op_b, args[2]),
            OP_ASTORES => format!("store_operand_s(1, {} + 2 * (glsi32) {}, {})", op_a, op_b, args[2]),
            OP_ASTOREB => format!("store_operand_b(1, {} + (glsi32) {}, {})", op_a, op_b, args[2]),
